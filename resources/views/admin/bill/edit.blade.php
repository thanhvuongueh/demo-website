        ...
        <div class="row">
            ...
            <div class="col-lg-7" style="padding-bottom:20px">
                    <div class="form-group">
                        ...
                        <input class="form-control" name="txtFname" placeholder="Please Enter First Name" value="{{$bill->first_name}}" />
                    </div>
                    <div class="form-group">
                        <label>Last Name</label>
                        <input class="form-control" name="txtLname" placeholder="Please Enter Last Name" value="{{$bill->last_name}}" />
                    </div>
                    <div class="form-group">
                        <label>Phone</label>
                        <input class="form-control" name="txtPhone" placeholder="Please Enter Phone" value="{{$bill->phone}}" />
                    </div>
                    <div class="form-group">
                        <label>Address</label>
                        <textarea class="form-control" name="txtAddress" cols="30" rows="3" placeholder="Please Enter Address">{{$bill->address}}</textarea>
                    </div>
            ...
            <div class="col-lg-12" style="margin-bottom: 20px">
                        ...
                        <tr>
                            ...
                            <td>
                                <select class="form-control sltCate" name="sltCate[]" >
                                    ...
                                    <?php showOptionSelect($allCates, $detail->product->cate->id)?>
                                </select>
                            </td>
                            <td>
                                <select class="form-control sltName" name="sltName[]">
                                    @foreach($detail->product->cate->product as $product)
                                        <option value="{{$product->id}}" @if($product->id == $detail->product_id) selected @endif>{{$product->name}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>{{number_format($detail->price)}}</td>
                            <td><input type="number" class="form-control txtQuality" name="txtQuality[]" value={{$detail->quality}} min=1></td>
                            <td>{{number_format($detail->price * $detail->quality)}}</td>
                ...
                <div class="text-right" style="font-size: 20px; font-weight: bold">
                    Total Bill: <span id="total-bill"> {{number_format($bill->total_price)}} </span>
                </div>
...
$(document).ready(function(){
    var stt = 1;
    var totalBill = parseInt($("#total-bill").html().split(",").join(""));
